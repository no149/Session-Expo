...
//TODO import { MessageReactsSelectorProps } from '../../components/conversation/message/message-content/MessageReactions';
...
const getMessagesOfSelectedConversation = createSelector(
  getConversations,
  (state: ConversationsStateType): Array<MessageModelPropsWithoutConvoProps> =>
    state.messages,
)
...
export const hasSelectedConversationIncomingMessages = createSelector(
  ...
  (messages: Array<MessageModelPropsWithoutConvoProps>): boolean => {
    ...
    return Boolean(
      messages.filter((m) => m.propsForMessage.direction === 'incoming').length,
    )
  },
)
...
export const getConversationHasUnread = createSelector(
  ...
  (sortedMessages, firstUnreadId) => {
    ...
    return sortedMessages.map((msg, index) => {
      ...
      const messageTimestamp =
        msg.propsForMessage.serverTimestamp || msg.propsForMessage.timestamp
      ...
      if (msg.propsForDataExtractionNotification) {
        return {
          ...
          message: {
            ...
            props: {
              ...msg.propsForDataExtractionNotification,
...
function getConversationTitle(
  ...
  if (conversation.type === 'group') {
    //TODO return (testingi18n || window.i18n)('unknown');
    return 'testingi18n'
  }
...
//TODO export const getConversationComparator = createSelector(getIntl, _getConversationComparator);
...
const _getUnreadConversationRequests = (
  ...
  //TODO return filter(sortedConversationRequests, conversation => {
...
//     return [
//       ...extractedContactsWithDisplayName,
//       ...extractedContactsNoDisplayName,
//     ]
//   },
// )
...
export const getConversationHeaderTitleProps = createSelector(
  getSelectedConversation,
  //TODO (state): ConversationHeaderTitleProps | undefined => {
  (state): {} => {
    if (!state) {
 ...
 */
export const getCurrentNotificationSettingText = createSelector(
  getSelectedConversation,
  (state): string | undefined => {
    ...
    switch (state.currentNotificationSetting) {
      case 'all':
        //TODO return window.i18n('notificationForConvo_all')
        return 'notificationForConvo_all'
      case 'mentions_only':
        //TODO return window.i18n('notificationForConvo_mentions_only')
        return 'notificationForConvo_mentions_only'
      case 'disabled':
        //TODO return window.i18n('notificationForConvo_disabled')
        return 'notificationForConvo_disabled'
      default:
        //TODO return window.i18n('notificationForConvo_all')
        return 'notificationForConvo_all'
    }
...
export const getNumberOfPinnedConversations = createSelector(
  getConversations,
  (state): number => {
...
export const getMessageDetailsViewProps = createSelector(
  getConversations,
  (state: ConversationsStateType): MessagePropsDetails | undefined =>
    state.messageDetailProps,
)
...
export const isMessageSelectionMode = createSelector(
  getConversations,
  (state: ConversationsStateType): boolean =>
    Boolean(state.selectedMessageIds.length > 0),
)
...
export const getLightBoxOptions = createSelector(
  getConversations,
  //TODO (state: ConversationsStateType): LightBoxOptions | undefined =>
  (state: ConversationsStateType): undefined => state.lightBox,
)

export const getQuotedMessage = createSelector(
  getConversations,
  //TODO (state: ConversationsStateType): ReplyingToMessageProps | undefined =>
  (state: ConversationsStateType): undefined => state.quotedMessage,
)
...
export const getQuotedMessageToAnimate = createSelector(
  getConversations,
  (state: ConversationsStateType): string | undefined =>
    state.animateQuotedMessageId || undefined,
)
  ...
  for (let i = 0; i < messageModelsProps.length; i++) {
    ...
    const previousSender =
      i < messageModelsProps.length - 1
        ...
        : undefined
    const nextSender =
      i > 0 ? messageModelsProps[i - 1].propsForMessage?.sender : undefined
...
function sortMessages(
  ...
  if (isPublic) {
    return messages.slice().sort((a, b) => {
      return (
        (b.propsForMessage.serverTimestamp || 0) -
        (a.propsForMessage.serverTimestamp || 0)
      )
    })
  }
  if (
    messages.some(
      (n) => !n.propsForMessage.timestamp && !n.propsForMessage.receivedAt,
    )
  ) {
...
export const getOldestMessageId = createSelector(
  ...
  (messages: Array<MessageModelPropsWithoutConvoProps>): string | undefined => {
    const oldest =
      messages.length > 0
        ? messages[messages.length - 1].propsForMessage.id
        : undefined
...
export const getYoungestMessageId = createSelector(
  ...
  (messages: Array<MessageModelPropsWithoutConvoProps>): string | undefined => {
    const youngest =
      messages.length > 0 ? messages[0].propsForMessage.id : undefined
...
export const getMessagePropsByMessageId = createSelector(
  ...
  (
    ...
    const foundMessageProps:
      | SortedMessageModelProps
      | undefined = messages?.find((m) => m?.propsForMessage?.id === id)
    ...
    const foundMessageConversation =
      conversations[foundMessageProps.propsForMessage.convoId]
    ...
    const groupModerators =
      (isGroup && foundMessageConversation.groupModerators) || []
    ...
    const authorProfileName = senderIsUs
      ? //TODO: window.i18n('you')
        'you'
      : foundSenderConversation.nickname ||
        foundSenderConversation.displayNameInProfile ||
        //TODO window.i18n('anonymous')
        'anonymous'
...
export const getMessageAvatarProps = createSelector(
  getMessagePropsByMessageId,
  //TODO (props): MessageAvatarSelectorProps | undefined => {
  (props): any => {
    if (!props || isEmpty(props)) {
      return undefined
    }

    //TODO const messageAvatarProps: MessageAvatarSelectorProps = {
    const messageAvatarProps: any = {
...
export const getMessageReactsProps = createSelector(
  getMessagePropsByMessageId,
  //TODO (props): MessageReactsSelectorProps | undefined => {
  (props): any | undefined => {
    if (!props || isEmpty(props)) {
      return undefined
    }

    //TODO const msgProps: MessageReactsSelectorProps = pick(props.propsForMessage, [
    const msgProps: any = pick(props.propsForMessage, [
      'convoId',
      'conversationType',
      'isPublic',
      'reacts',
...
export const getMessageLinkPreviewProps = createSelector(
  getMessagePropsByMessageId,
  //TODO (props): MessageLinkPreviewSelectorProps | undefined => {
  (props): any | undefined => {
    if (!props || isEmpty(props)) {
      return undefined
    }

    //TODO const msgProps: MessageLinkPreviewSelectorProps = pick(
    const msgProps: any = pick(props.propsForMessage, [
      'direction',
      'attachments',
      'previews',
...
export const getMessageQuoteProps = createSelector(
  getMessagePropsByMessageId,
  //TODO (props): MessageQuoteSelectorProps | undefined => {
  (props): any | undefined => {
    if (!props || isEmpty(props)) {
      return undefined
    }

    //TODO const msgProps: MessageQuoteSelectorProps = pick(props.propsForMessage, [
    const msgProps: any = pick(props.propsForMessage, ['direction', 'quote'])

    return msgProps
  },
)

export const getMessageStatusProps = createSelector(
  getMessagePropsByMessageId,
  //TODO (props): MessageStatusSelectorProps | undefined => {
  (props): any | undefined => {
    if (!props || isEmpty(props)) {
      return undefined
    }

    //TODO const msgProps: MessageStatusSelectorProps = pick(props.propsForMessage, [
    const msgProps: any = pick(props.propsForMessage, ['direction', 'status'])
...
export const getMessageTextProps = createSelector(
  getMessagePropsByMessageId,
  //TODO (props): MessageTextSelectorProps | undefined => {
  (props): any | undefined => {
    if (!props || isEmpty(props)) {
      return undefined
    }

    //TODO const msgProps: MessageTextSelectorProps = pick(props.propsForMessage, [
    const msgProps: any = pick(props.propsForMessage, [
      'direction',
      'status',
      'text',
...
export const getMessageContextMenuProps = createSelector(
  getMessagePropsByMessageId,
  //TODO (props): MessageContextMenuSelectorProps | undefined => {
  (props): any | undefined => {
    if (!props || isEmpty(props)) {
      return undefined
    }

    //TODO const msgProps: MessageContextMenuSelectorProps = pick(
    const msgProps: any = pick(props.propsForMessage, [
      'attachments',
      'sender',
      'convoId',
...
export const getMessageAuthorProps = createSelector(
  getMessagePropsByMessageId,
  //TODO (props): MessageAuthorSelectorProps | undefined => {
  (props): any | undefined => {
    if (!props || isEmpty(props)) {
      return undefined
    }

    //TODO const msgProps: MessageAuthorSelectorProps = {
    const msgProps: any = {
...
export const getMessageAttachmentProps = createSelector(
  getMessagePropsByMessageId,
  //TODO (props): MessageAttachmentSelectorProps | undefined => {
  (props): any | undefined => {
    if (!props || isEmpty(props)) {
      return undefined
    }

    //TODO const msgProps: MessageAttachmentSelectorProps = {
    const msgProps: any = {
...
export const getMessageContentSelectorProps = createSelector(
  getMessagePropsByMessageId,
  //TODO (props): MessageContentSelectorProps | undefined => {
  (props): any | undefined => {
    if (!props || isEmpty(props)) {
      return undefined
    }

    //TODO const msgProps: MessageContentSelectorProps = {
    const msgProps: any = {
      ...pick(props.propsForMessage, [
        'direction',
        'serverTimestamp',
...
export const getMessageContentWithStatusesSelectorProps = createSelector(
  getMessagePropsByMessageId,
  //TODO (props): MessageContentWithStatusSelectorProps | undefined => {
  (props): any | undefined => {
    if (!props || isEmpty(props)) {
      return undefined
    }

    //TODO const msgProps: MessageContentWithStatusSelectorProps = {
    const msgProps: any = {
...
export const getGenericReadableMessageSelectorProps = createSelector(
  getMessagePropsByMessageId,
  //TODO (props): GenericReadableMessageSelectorProps | undefined => {
  (props): any | undefined => {
    if (!props || isEmpty(props)) {
      return undefined
    }

    //TODO const msgProps: GenericReadableMessageSelectorProps = pick(
    const msgProps: any = pick(props.propsForMessage, [
      'convoId',
      'direction',
      'conversationType',
...
export const getIsSelectedConvoInitialLoadingInProgress = createSelector(
  getSelectedConversation,
  (convo: ReduxConversationType | undefined): boolean =>
    Boolean(convo?.isInitialFetchingInProgress),
)
