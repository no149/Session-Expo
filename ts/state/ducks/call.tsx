  ...
  reducers: {
    ...
    answerCall(
      ...
      if (
        ...
        //TODO window.log.info('cannot answer a call we are not displaying a dialog with');
        return state
      }
    ...
    callConnected(
      ...
      if (callerPubkey !== state.ongoingWith) {
        //TODO window.log.info('cannot answer a call we did not start or receive first');
        return state
      }
      const existingCallState = state.ongoingCallStatus

      if (
        existingCallState !== 'connecting' &&
        existingCallState !== 'offering'
      ) {
    ...
    callReconnecting(
      ...
      const callerPubkey = action.payload.pubkey
      if (callerPubkey !== state.ongoingWith) {
        //TODO window.log.info('cannot reconnect a call we did not start or receive first');
        return state
      }
      const existingCallState = state.ongoingCallStatus

      if (existingCallState !== 'ongoing') {
        //TODO window.log.info('cannot reconnect a call we are not ongoing');
        return state
      }
    ...
    startingCallWith(
      ...
      if (state.ongoingWith) {
        //TODO window.log.warn('cannot start a call with an ongoing call already: ongoingWith');
        return state
      }
      if (state.ongoingCallStatus) {
        //TODO window.log.warn('cannot start a call with an ongoing call already: ongoingCallStatus');
        return state
      }
      setIsRinging(true)
