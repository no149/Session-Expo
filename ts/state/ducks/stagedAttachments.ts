...
//TODO import { StagedAttachmentType } from '../../components/conversation/composition/CompositionBox';
...
const stagedAttachmentsSlice = createSlice({
  ...
  reducers: {
    addStagedAttachmentsInConversation(
      ...
      const currentStagedAttachments =
        state.stagedAttachments[conversationKey] || []

      if (
        newAttachments.some((a) => a.isVoiceMessage) &&
        currentStagedAttachments.length > 0
      ) {
        //TODO   window?.log?.warn('A voice note cannot be sent with other attachments');
        return state
      }
    ...
    removeStagedAttachmentInConversation(
      ...
      state.stagedAttachments[conversationKey] = state.stagedAttachments[
        conversationKey
