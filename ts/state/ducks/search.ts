...
//TODO import { MessageResultProps } from '../../components/search/MessageSearchResults';
...
async function doSearch(query: string): Promise<SearchResultsPayloadType> {
  const options: SearchOptions = {
    //TODO noteToSelf: window.i18n('noteToSelf').toLowerCase(),
...
function getUnixMillisecondsTimestamp(timestamp: string): number {
  ...
  if (!isNaN(timestampInt)) {
    try {
      ...
      //TODO window?.log?.warn('Advanced Search: ', error);
...
async function queryMessages(
  ...
  try {
    ...
    //TODO window.log.warn('queryMessages failed with', e.message);
    return []
  }
}
...
async function queryConversationsAndContacts(
  ...
  const searchResults: Array<ReduxConversationType> = await Data.searchConversations(
    query,
  )
