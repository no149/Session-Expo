...
export async function copyPublicKeyByConvoId(convoId: string) {
  if (convoId.match(openGroupPrefixRegex)) {
    ...
    if (convoId.match(openGroupV2ConversationIdRegex)) {
      ...
      if (completeUrl) {
        //TODO window.clipboard.writeText(completeUrl)

        ToastUtils.pushCopiedToClipBoard()
        return
      }
      //TODO window?.log?.warn('copy to pubkey no roomInfo')
      return
    }
  ...
  //TODO window.clipboard.writeText(convoId)
...
export async function blockConvoById(conversationId: string) {
  ...
  //TODO ToastUtils.pushToastSuccess('blocked', window.i18n('blocked'))
}
...
export async function unblockConvoById(conversationId: string) {
  ...
  if (!conversation) {
    ...
    //TODO ToastUtils.pushToastSuccess('unblocked', window.i18n('unblocked'))
    return
  }
  if (!conversation.id || conversation.isPublic()) {
    return
...
export const approveConvoAndSendResponse = async (
  ...
  if (!convoToApprove || convoToApprove.isApproved()) {
    //TODO window?.log?.info('Conversation is already approved.')
    return
  }
...
export const declineConversationWithoutConfirm = async (
  ...
  if (!conversationToDecline || conversationToDecline.isApproved()) {
    //TODO window?.log?.info('Conversation is already declined.')
    return
  }
...
export async function showUpdateGroupNameByConvoId(conversationId: string) {
  ...
  //TODO window.inboxStore?.dispatch(updateGroupNameModal({ conversationId }))
}
...
export async function showUpdateGroupMembersByConvoId(conversationId: string) {
  ...
  //TODO window.inboxStore?.dispatch(updateGroupMembersModal({ conversationId }))
}
...
export function showLeaveGroupByConvoId(conversationId: string) {
  ...
  //TODO const title = window.i18n('leaveGroup')
  //TODO const message = window.i18n('leaveGroupConfirmation')
  ...
  if (!isClosedGroup || (isClosedGroup && !isAdmin)) {
    const onClickClose = () => {
      //TODO window.inboxStore?.dispatch(updateConfirmModal(null))
    }
    //TODO window.inboxStore?.dispatch(
    //   updateConfirmModal({
    //     title,
...
export function showInviteContactByConvoId(conversationId: string) {
  //TODO window.inboxStore?.dispatch(updateInviteContactModal({ conversationId }))
}

export function showAddModeratorsByConvoId(conversationId: string) {
  //TODO window.inboxStore?.dispatch(updateAddModeratorsModal({ conversationId }))
}

export function showRemoveModeratorsByConvoId(conversationId: string) {
  //TODO window.inboxStore?.dispatch(updateRemoveModeratorsModal({ conversationId }))
}
...
export function showUnbanUserByConvoId(
  conversationId: string,
  pubkey?: string,
) {
  //TODO window.inboxStore?.dispatch(
...
export function showChangeNickNameByConvoId(conversationId: string) {
  //TOOD window.inboxStore?.dispatch(changeNickNameModal({ conversationId }))
}
...
export async function setDisappearingMessagesByConvoId(
  ...
  const canSetDisappearing =
    !conversation.isOutgoingRequest() && !conversation.isIncomingRequest()
...
export async function uploadOurAvatar(newAvatarDecrypted?: ArrayBuffer) {
  const ourConvo = getConversationController().get(
    UserUtils.getOurPubKeyStrFromCache(),
  )
  if (!ourConvo) {
    //TODO window.log.warn('ourConvo not found... This is not a valid case')
    return
  }
  ...
  if (newAvatarDecrypted) {
    ...
    if (!profileKey) {
      //TODO window.log.info('our profileKey not found. Not reuploading our avatar')
      return
    }
    ...
    if (!currentAttachmentPath) {
      ...
      //   'No attachment currently set for our convo.. Nothing to do.',
      // )
      return
    }
    ...
    if (!decryptedAvatarUrl) {
      //TODO window.log.warn('Could not decrypt avatar stored locally..')
      return
    }
  ...
  if (!decryptedAvatarData?.byteLength) {
    //TOOD window.log.warn('Could not read content of avatar ...')
    return
  }
  ...
  if (!avatarPointer) {
    //TODO window.log.warn('failed to upload avatar to fileserver')
    return
  }
  ...
  await Data.createOrUpdateItem({
    id: lastAvatarUploadTimestamp,
    value: newTimestampReupload,
  })
...
export async function replyToMessage(messageId: string) {
  ...
  const quotedMessageProps = await conversationModel.makeQuote(
    quotedMessageModel,
  )

  if (quotedMessageProps) {
    //TODO window.inboxStore?.dispatch(quoteMessage(quotedMessageProps))
  } else {
    //TODO window.inboxStore?.dispatch(quoteMessage(undefined))
  }
}
...
export async function showLinkSharingConfirmationModalDialog(e: any) {
  ...
  if (
    isURL(pastedText)
    //TODO && !window.getSettingValue('link-preview-setting', false)
  ) {
    ...
    if (!alreadyDisplayedPopup) {
      ...
      //     onClickClose: async () => {
      //       await Data.createOrUpdateItem({
      //         id: hasLinkPreviewPopupBeenDisplayed,
      //         value: true,
      //       })
      //     },
      //   }),
