...
const conversations_2 = require("../session/conversations");
async function destroyMessagesAndUpdateRedux(messages) {
    ...
    window.inboxStore?.dispatch((0, conversations_1.messagesExpired)(messages));
    conversationWithChanges.map(convoIdToUpdate => {
        (0, conversations_2.getConversationController)()
            .get(convoIdToUpdate)
            ?.updateLastMessage();
    });
}
