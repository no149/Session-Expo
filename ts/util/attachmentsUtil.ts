 ...
 */
export async function autoScaleForAvatar<
  T extends { contentType: string; blob: Blob }
>(attachment: T) {
  ...
  if (
    ...
    // nothing to do
    throw new Error(
      'Cannot autoScaleForAvatar another file than PNG,GIF or JPEG.',
    )
  }

  //TODO window.log.info('autoscale for avatar', maxMeasurements);
...
export async function autoScaleForIncomingAvatar(incomingAvatar: ArrayBuffer) {
  ...
  const contentType =
    imageType(new Uint8Array(incomingAvatar))?.mime || IMAGE_UNKNOWN
  ...
  //TODO window.log.info('autoscale for incoming avatar', maxMeasurements);
...
>(attachment: T) {
  ...
  //TODO window.log.info('autoScaleForThumbnail', maxMeasurements);
...
export async function autoScale<T extends { contentType: string; blob: Blob }>(
  ...
  //TODO window.log.debug('canvas.originalWidth', {
  ...
  do {
    i -= 1
    //TODO window.log.info(`autoscale iteration: [${i}] for:`, attachment);

    perfStart(`autoscale-canvasToBlob-${attachment.blob.size}`)
    const tempBlob = await canvasToBlob(
    ...
    quality =
      (quality * maxSize) / (readAndResizedBlob.size * (i === 1 ? 2 : 1)) // make the last iteration decrease drastically quality of the image
  ...
  if (readAndResizedBlob.size > maxSize) {
    throw new Error(
      'Cannot add this attachment even after trying to scale it down.',
    )
  }
  //TODO  window.log.debug(`[perf] autoscale took ${Date.now() - start}ms `);
...
//TODO export type StagedAttachmentImportedType = Omit<
 ...
 */
//TODO export type StagedImagePreviewImportedType = Pick<
//   StagedAttachmentType,
 ...
 */
//TODO export type StagedPreviewImportedType = {
//   url: string;
//   title: string;
//   image?: StagedImagePreviewImportedType;
// };

//TODO export async function getFileAndStoreLocally(
...
//TODO const attachmentFlags = attachment.isVoiceMessage
...
export async function getFileAndStoreLocallyImageBuffer(
  ...
  const contentType =
    imageType(new Uint8Array(imageBuffer))?.mime || IMAGE_UNKNOWN
...
export async function readAvatarAttachment(attachment: {
  ...
  return {
    attachment,
    data: dataReadFromBlob,
    size: dataReadFromBlob.byteLength,
  }
}
...
export const saveAttachmentToDisk = async ({
  ...
  const decryptedUrl = await getDecryptedMediaUrl(
    attachment.url,
