...
export async function downloadAttachment(attachment: {
  ...
  if (defaultFileServer) {
    ...
    //TODO window?.log?.info('Download v2 file server attachment', attachmentId);
    ...
    // );
    throw new Error(
      'Attachment url is not opengroupv2 nor fileserver. Unsupported',
    )
  }

  if (!res?.byteLength) {
    //TODO window?.log?.error('Failed to download attachment. Length is 0');
    throw new Error(
  ...
  if (!attachment.isRaw) {
    ...
    const keyBuffer = (await callUtilsWorker(
...
export async function downloadAttachmentSogsV3(
  ...
  if (!roomDetails) {
    throw new Error(
      `Didn't find such a room ${roomInfos.serverUrl}: ${roomInfos.roomId}`,
    )
  }
  ...
  if (attachment.size !== dataUint.length) {
    ...
    //TODO window?.log?.info('Received opengroupv2 unpadded attachment size:', attachment.size);
  }
...
async function processNormalAttachments(
  ...
  if (message.isTrustedForAttachmentDownload()) {
    const openGroupV2Details =
      (isOpenGroupV2 && convo.toOpenGroupV2()) || undefined
  ...
  //TODO window.log.info('No downloading attachments yet as this user is not trusted for now.');
  return 0
}
