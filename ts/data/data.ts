...
export const hasLinkPreviewPopupBeenDisplayed =
...
function _cleanData(data: any): any {
  ...
  for (let index = 0, max = keys.length; index < max; index += 1) {
    ...
    if (_.isFunction(value.toNumber)) {
      ...
      //TODO window?.log?.info(`_cleanData: key ${key} had type ${typeof value}`)
    }
  }
  return data
}
...
async function removeAllClosedGroupEncryptionKeyPairs(
  groupPublicKey: string,
): Promise<void> {
 ...
 */
async function getPubkeysInPublicConversation(
  id: string,
...
//TODO async function searchMessages(
//   query: string,
//   limit: number,
// ): Promise<Array<MessageResultProps>> {
//   const messages = (await channels.searchMessages(query, limit)) as Array<
//     MessageResultProps
//   >
...
async function filterAlreadyFetchedOpengroupMessage(
  ...
  const msgDetailsNotAlreadyThere = await channels.filterAlreadyFetchedOpengroupMessage(
    msgDetails,
  )
...
// might throw
async function getUnreadCountByConversation(
  conversationId: string,
): Promise<number> {
...
async function getMessagesByConversation(
  conversationId: string,
  {
    skipTimerInit = false,
    messageId = null,
  }: { skipTimerInit?: false; messageId: string | null },
): Promise<MessageCollection> {
...
async function getUnprocessedById(
  id: string,
): Promise<UnprocessedParameter | undefined> {
...
async function updateSnodePoolOnDb(snodesAsJsonString: string): Promise<void> {
  await Data.createOrUpdateItem({
    id: SNODE_POOL_ITEM_ID,
    value: snodesAsJsonString,
  })
}
...
async function fillWithTestData(convs: number, msgs: number) {
  ...
  for (let msgsAddedCount = 0; msgsAddedCount < msgs; msgsAddedCount++) {
    ...
    const convoToChoose =
      newConvos[Math.floor(Math.random() * newConvos.length)]
