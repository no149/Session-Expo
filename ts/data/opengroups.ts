  ...
  if (loadedFromDB) {
    ...
    loadedFromDB.forEach((r) => {
      try {
        ...
        //TODO window.log.warn(e.message);
      }
    })
    return
  }
...
function getV2OpenGroupRoom(
  conversationId: string,
): OpenGroupV2Room | undefined {
  if (!isOpenGroupV2(conversationId)) {
    throw new Error(
      `getV2OpenGroupRoom: this is not a valid v2 id: ${conversationId}`,
    )
  }
...
function getV2OpenGroupRoomsByServerUrl(
  serverUrl: string,
...
async function saveV2OpenGroupRooms(
  rooms: Array<OpenGroupV2Room>,
): Promise<void> {
...
async function saveV2OpenGroupRoom(room: OpenGroupV2Room): Promise<void> {
  ...
  const found =
    (room.conversationId &&
      throwIfNotLoaded().find(
        (m) => m.conversationId === room.conversationId,
      )) ||
    undefined
...
async function removeV2OpenGroupRoom(conversationId: string): Promise<void> {
  ...
  const foundIndex =
    conversationId &&
    throwIfNotLoaded().findIndex((m) => m.conversationId === conversationId)
  if (isNumber(foundIndex) && foundIndex > -1) {
    throwIfNotLoaded().splice(foundIndex, 1)
  }
}

async function getAllOpenGroupV2Conversations(): Promise<
