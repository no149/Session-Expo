//TODO import { ipcRenderer } from 'electron';
...
export async function shutdown() {
  ...
  //TODO window?.log?.info(`data.shutdown: starting process. ${jobKeys.length} jobs outstanding`);
  ...
  _shutdownPromise = new Promise((resolve, reject) => {
    _shutdownCallback = (error: any) => {
      //TODO window?.log?.info('data.shutdown: process complete');
      if (error) {
...
function makeChannel(fnName: string) {
  channels[fnName] = async (...args: any) => {
    ...
    return new Promise((resolve, reject) => {
      //TODO ipcRenderer.send(SQL_CHANNEL_KEY, jobId, fnName, ...args);
...
export function initData() {
  ...
  //TODO ipcRenderer.setMaxListeners(0);

  channelsToMake.forEach(makeChannel)

  //TODO ipcRenderer.on(`${SQL_CHANNEL_KEY}-done`, (_event, jobId, errorForDisplay, result) => {
  ...
  jobs[id] = {
    ...
    resolve: (value: any) => {
      ...
      if (_DEBUG) {
        ...
        if (delta > 10) {
          //TODO window?.log?.debug(`SQL channel job ${id} (${fnName}) succeeded in ${end - start}ms`);
        }
      }
    ...
    reject: (error: any) => {
      removeJob(id)
      const end = Date.now()
      //TODO window?.log?.warn(`SQL channel job ${id} (${fnName}) failed in ${end - start}ms`);
      return reject(error)
    },
  }
}
...
function makeJob(fnName: string) {
  ...
  if (_DEBUG) {
    //TODO window?.log?.debug(`SQL channel job ${id} (${fnName}) started`);
  }
  jobs[id] = {
    fnName,
    start: Date.now(),
