...
import {
  createDeleter,
  getAttachmentsPath,
} from '../shared/attachments/shared_attachments'
import {
...
let internalReadAttachmentData:
  | ((relativePath: string) => Promise<ArrayBufferLike>)
  | undefined
...
export const processNewAttachment = async (attachment: {
  ...
  const fileName = attachment.fileName
    ? replaceUnicodeV2(attachment.fileName)
    : ''
  ...
  const attachmentWithoutData = omit(
    { ...attachment, fileName, path: onDiskAttachmentPath },
    ['data'],
  )
...
export const readAttachmentData = async (
  relativePath: string,
...
export const migrateDataToFileSystem = async (data?: ArrayBuffer) => {
  ...
  if (!isValidData) {
    throw new TypeError(
      `Expected ${data} to be an array buffer got: ${typeof data}`,
    )
  }
