...
export function arrayContainsOneItemOnly(
  arrayToCheck: Array<string> | undefined,
) {
...
export class MessageModel extends Backbone.Model<MessageAttributes> {
  constructor(
    attributes: MessageAttributesOptionals & { skipTimerInit?: boolean },
  ) {
  ...
  public isExpirationTimerUpdate() {
    ...
    return (
      Boolean(flags & expirationTimerFlag) || !isEmpty(expirationTimerUpdate)
    )
  }
  ...
  public getPropsForTimerNotification(): PropsForExpirationTimer | null {
    ...
    const basicProps: PropsForExpirationTimer = {
      ...
      type: fromSync
        ? 'fromSync'
        : UserUtils.isUsFromCache(source)
        ? 'fromMe'
        : 'fromOther',
      messageId: this.id,
  ...
  public getPropsForDataExtractionNotification(): PropsForDataExtractionNotification | null {
    ...
    return {
      ...
      name:
        contact.profileName ||
        contact.name ||
        dataExtractionNotification.source,
      messageId: this.id,
      receivedAt: this.get('received_at'),
      isUnread: this.isUnread(),
    }
  ...
  public getPropsForMessageRequestResponse(): PropsForMessageRequestResponse | null {
    ...
    return {
      ...
      name:
        contact.profileName || contact.name || messageRequestResponse.source,
      messageId: this.id,
      receivedAt: this.get('received_at'),
      isUnread: this.isUnread(),
  ...
  public getPropsForMessage(
    ...
    const props: PropsForMessageWithoutConvoProps = {
      id: this.id,
      direction: (this.isIncoming()
        ? 'incoming'
        : 'outgoing') as MessageModelType,
  ...
  public getPropsForQuote(_options: any = {}) {
    ...
    const contact: ConversationModel =
      author && getConversationController().get(author)

    const authorName = contact
      ? contact.getContactProfileNameOrShortenedPubKey()
      : null

    let isFromMe = contact
    ...
    if (!referencedMessageNotFound) {
      if (quote.text) {
        ...
        quoteProps.text = this.createNonBreakingLastSeparator(
          sliceQuoteText(quote.text),
        )
      }
  ...
  public getPropsForAttachment(
    attachment: AttachmentTypeWithPath,
  ): PropsForAttachment | null {
    if (!attachment) {
      return null
    }
  ...
  public async uploadData() {
    ...
    if (conversation?.isPublic()) {
      ...
      linkPreviewPromise = uploadLinkPreviewsV3(
        firstPreviewWithData,
        openGroupV2,
      )
    ...
    window.log.info(
      `Upload of message data for message ${this.idForLogging()} is finished.`,
    )
    return {
      body,
      attachments,
  ...
  public async sendSyncMessageOnly(dataMessage: DataMessage) {
    ...
    const data =
      dataMessage instanceof DataMessage ? dataMessage.dataProto() : dataMessage
  ...
  public findAndFormatContact(pubkey: string): FindAndFormatContactType {
    ...
    if (
      ...
      profileName = 'you' //TODO window.i18n('you');
      isMe = true
    } else {
  ...
  private getDescription() {
    ...
    if (groupUpdate) {
      ...
      if (
        ...
        //TODO return window.i18n('updatedTheGroup'); // Group Updated
        return 'updatedTheGroup' // Group Updated
      }
      if (groupUpdate.name) {
        //TODO return window.i18n('titleIsNow', [groupUpdate.name]);
        return 'titleIsNow'
      }
      if (groupUpdate.joined && groupUpdate.joined.length) {
        ...
        if (names.length > 1) {
          ...
          messages.push('multipleJoinedTheGroup')
        } else {
          //TODO messages.push(window.i18n('joinedTheGroup', names));
          messages.push('joinedTheGroup')
        }
      ...
      if (groupUpdate.kicked && groupUpdate.kicked.length) {
        ...
        if (names.length > 1) {
          //TODO messages.push(window.i18n('multipleKickedFromTheGroup', [names.join(', ')]));
          messages.push('multipleKickedFromTheGroup')
        } else {
          //TODO messages.push(window.i18n('kickedFromTheGroup', names));
    ...
    if (this.isIncoming() && this.hasErrors()) {
      //TODO return window.i18n('incomingError');
      return 'incomingError'
    }
    if (this.isGroupInvitation()) {
      //TODO return `😎 ${window.i18n('openGroupInvitation')}`;
...
const throttledAllMessagesDispatch = debounce(
  () => {
    ...
    //TODO window.inboxStore?.dispatch(messagesChanged([...updatesToDispatch.values()]));
