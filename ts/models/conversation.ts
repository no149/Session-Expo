...
export class ConversationModel extends Backbone.Model<ConversationAttributes> {
  ...
  constructor(attributes: ConversationAttributes) {
    ...
    this.updateLastMessage = throttle(
  ...
  public static hasValidOutgoingRequestValues({
    ...
    return Boolean(
      !isMe && isApproved && isPrivate && !isBlocked && !didApproveMe,
    )
  }
  ...
  public isPublic(): boolean {
    return Boolean(
      this.id && this.id.match(OpenGroupUtils.openGroupPrefixRegex),
    )
  }
  ...
  public getGroupModerators(): Array<string> {
    const groupModerators = this.get('groupModerators') as
      | Array<string>
      | undefined

    return this.isPublic() && groupModerators && groupModerators?.length > 0
      ? groupModerators
      : []
  }
  ...
  public async updateGroupModerators(
    groupModerators: Array<string>,
    shouldCommit: boolean,
  ) {
    if (!this.isPublic()) {
  ...
  public async makeQuote(
  ...
  ): Promise<ReplyingToMessageProps | null> {
    ...
    if (!quotedMessage.get('sent_at')) {
      //TODO window.log.warn('tried to make a quote without a sent_at timestamp')
      return null
    }
    ...
    if (this.isPublic()) {
      ...
      if (
        room &&
        roomHasBlindEnabled(room) &&
        msgSource === UserUtils.getOurPubKeyStrFromCache()
      ) {
        ...
        msgSource = await findCachedOurBlindedPubkeyOrLookItUp(
  ...
  public async sendMessageJob(
    ...
    try {
      ...
      if (this.id.startsWith('15')) {
        //TODO window.log.info('Sending a blinded message to this user: ', this.id)
      ...
      if (this.isOpenGroupV2()) {
        const chatMessageOpenGroupV2 = new OpenGroupVisibleMessage(
          chatMessageParams,
        )
  ...
  public async sendReactionJob(
    ...
    try {
      ...
      //TODO window.log.error(`Reaction job failed id:${reaction.id} error:`, e)
      return null
    }
  }
  ...
  public async sendBlindedMessageRequest(messageParams: VisibleMessageParams) {
    ...
    if (!PubKey.hasBlindedPrefix(this.id)) {
      ...
      //   'sendBlindedMessageRequest - convo is not a blinded one',
      // )
      return
    }

    if (!messageParams.body) {
      //TODO window?.log?.warn('sendBlindedMessageRequest - needs a body')
      return
    }
    ...
    if (!roomInfo || !roomInfo.serverPublicKey) {
      ToastUtils.pushToastError(
        'no-sogs-matching',
        //TODO window.i18n('couldntFindServerMatching'),
        'couldntFindServerMatching',
      )
      //TODO window?.log?.error(
      //   'Could not find room with matching server url',
      //   groupUrl,
      // )
      throw new Error(
        `Could not find room with matching server url: ${groupUrl}`,
      )
    }
  ...
  public async updateExpireTimer(
    ...
    //TODO window?.log?.info("Update conversation 'expireTimer'", {
  ...
  public async addSingleOutgoingMessage(
    ...
    if (this.isPublic()) {
      ...
      if (
        ...
        if (!signingKeys) {
          throw new Error(
            'addSingleOutgoingMessage: getUserED25519KeyPairBytes returned nothing',
          )
        }
  ...
  public async leaveClosedGroup() {
    if (this.isMediumGroup()) {
      ...
      //TODO window?.log?.error('Cannot leave a non-medium group conversation')
      throw new Error(
  ...
  public async markAllAsRead() {
    ...
    if (this.isOpenGroupV2() || !expireTimerSet) {
      ...
      //TODO window.inboxStore?.dispatch(markConversationFullyRead(this.id))
  ...
  public async markReadBouncy(
    ...
    if (allProps.length) {
      //TODO window.inboxStore?.dispatch(conversationActions.messagesChanged(allProps))
    }
    ...
    const mentionRead = (() => {
      ...
      return !stillUnread.some(
        (m) => m.get('body')?.indexOf(`@${ourNumber}`) !== -1,
      )
    })()
  ...
  public async sendReadReceiptsIfNeeded(timestamps: Array<number>) {
    ...
    const settingsReadReceiptEnabled =
      Storage.get(SettingsKey.settingsReadReceipt) || false
    const sendReceipt =
      settingsReadReceiptEnabled &&
      !this.isBlocked() &&
      !this.isIncomingRequest()

    if (sendReceipt) {
      //TODO window?.log?.info(`Sending ${timestamps.length} read receipts.`)
  ...
  public async setNickname(nickname: string | null) {
    if (!this.isPrivate()) {
      //TODO window.log.info('cannot setNickname to a non private conversation.')
      return
    }
  ...
  public getNicknameOrRealUsernameOrPlaceholder(): string {
    ...
    if (this.isPrivate()) {
      //TODO return window.i18n('anonymous')
      return 'anonymous'
    }
    //TODO return window.i18n('unknown')
    return 'unknown'
  }
  ...
  public async setIsApproved(value: boolean, shouldCommit: boolean = true) {
    if (value !== this.isApproved()) {
      //TODO window?.log?.info(
      //   `Setting ${ed25519Str(this.id)} isApproved to: ${value}`,
      // )
      this.set({
        isApproved: value,
      })
  ...
  public async setDidApproveMe(value: boolean, shouldCommit: boolean = true) {
    if (value !== this.didApproveMe()) {
      //TODO window?.log?.info(
      //   `Setting ${ed25519Str(this.id)} didApproveMe to: ${value}`,
      // )
      this.set({
        didApproveMe: value,
      })
  ...
  public getAvatarPath(): string | null {
    ...
    if (avatar) {
      throw new Error(
        'avatarInProfile must be a string as we do not allow the {path: xxx} syntax',
      )
    }
  ...
  public async getNotificationIcon() {
    ...
    const decryptedAvatarUrl = await getDecryptedMediaUrl(
      avatarUrl,
      IMAGE_JPEG,
      true,
    )
    ...
    if (!decryptedAvatarUrl) {
      //TODO window.log.warn(
      //   'Could not decrypt avatar stored locally for getNotificationIcon..',
      // )
      return noIconUrl
    }
  ...
  public async notify(message: MessageModel) {
    ...
    if (!this.isApproved()) {
      //TODO window?.log?.info(
      //   'notification cancelled for unapproved convo',
      //   this.idForLogging(),
      // )
      ...
      const isFirstMessageOfConvo =
        (await Data.getMessagesByConversation(this.id, { messageId: null }))
          .length === 1
      if (hadNoRequestsPrior && isFirstMessageOfConvo) {
        //TODO friendRequestText = window.i18n('youHaveANewFriendRequest')
    ...
    if (convNotif === 'disabled') {
      //TODO window?.log?.info('notifications disabled for convo', this.idForLogging())
      return
    }
    if (convNotif === 'mentions_only') {
      ...
      const mentionMe =
        mentions && mentions.some((m) => isUsAnySogsFromCache(m.slice(1)))
  ...
  public async notifyIncomingCall() {
    if (!this.isPrivate()) {
      //TODO window?.log?.info(
      //   'notifyIncomingCall: not a private convo',
      //   this.idForLogging(),
      // )
      return
    }
  ...
  private async addSingleMessage(
    ...
    //TODO window.inboxStore?.dispatch(
    //   conversationActions.messagesChanged([messageModelProps]),
    // )
  ...
  private sendTypingMessage(isTyping: boolean) {
    ...
    getMessageQueue().sendToPubKey(device, typingMessage)
    //TODO .catch(window?.log?.error)
  }
    ...
    if (modsOrAdmins && isArray(modsOrAdmins)) {
      ...
      const moderatorsOrAdminsChanged =
        type === 'admins'
          ...
          : await this.updateGroupModerators(
...
const throttledAllConversationsDispatch = debounce(
  () => {
    ...
    //TODO window.inboxStore?.dispatch(
    //   conversationsChanged([...updatesToDispatch.values()]),
    // )
