...
export const getUserED25519KeyPairBytes = async (): Promise<
  ByteKeyPair | undefined
> => {
  ...
  if (ed25519KeyPair?.publicKey && ed25519KeyPair?.privateKey) {
    const pubKeyBytes = new Uint8Array(
      _.map(ed25519KeyPair.publicKey, (a) => a),
    )
    const privKeyBytes = new Uint8Array(
      _.map(ed25519KeyPair.privateKey, (a) => a),
    )
    return {
      pubKeyBytes,
      privKeyBytes,
...
export function getOurProfile(): LokiProfile | undefined {
  try {
    ...
    const profileKeyAsBytes = ourProfileKeyHex
