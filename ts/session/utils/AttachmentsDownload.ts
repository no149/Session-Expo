...
import {
  downloadAttachment,
  downloadAttachmentSogsV3,
} from '../../receiver/attachments'
import {
  initializeAttachmentLogic,
...
async function _runJob(job: any) {
  ...
  try {
    ...
    if (
      isOpenGroupV2 &&
      (!openGroupV2Details?.serverUrl || !openGroupV2Details.roomId)
    ) {
    ...
    try {
      ...
      if (isOpenGroupV2) {
        downloaded = await downloadAttachmentSogsV3(
          attachment,
          openGroupV2Details,
        )
      } else {
    ...
    if (!attachment.contentType) {
      //TODO window.log.warn('incoming attachment has no contentType');
    }
