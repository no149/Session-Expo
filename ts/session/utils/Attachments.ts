...
async function uploadToFileServer(
  params: UploadParams,
): Promise<AttachmentPointerWithUrl> {
  ...
  //TODO window?.log?.warn('upload to file server v2 failed');
...
export async function uploadLinkPreviewToFileServer(
  ...
  if (!preview?.image) {
    //TODO window.log.warn('tried to upload file to FileServer without image.. skipping');
