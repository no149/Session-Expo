...
export type CallManagerListener =
  | ((options: CallManagerOptionsType) => void)
  | null
...
export function addVideoEventsListener(
  uniqueId: string,
  listener: CallManagerListener,
) {
...
export async function selectCameraByDeviceId(cameraDeviceId: string) {
  ...
  if (camerasList.some((m) => m.deviceId === cameraDeviceId)) {
    ...
    try {
      ...
      if (!peerConnection) {
        throw new Error(
          'cannot selectCameraByDeviceId without a peer connection',
        )
      }

  //TODO    window.log.info('replacing video track')
      ...
      //TODO window.log.warn('selectCameraByDeviceId failed with', e.message)
...
export async function selectAudioInputByDeviceId(audioInputDeviceId: string) {
  ...
  if (audioInputsList.some((m) => m.deviceId === audioInputDeviceId)) {
    ...
    try {
      ...
      if (!peerConnection) {
        throw new Error(
          'cannot selectAudioInputByDeviceId without a peer connection',
        )
      }
      const audioSender = peerConnection.getSenders().find((s) => {
        return s.track?.kind === audioTrack.kind
      })
      //TODO window.log.info('replacing audio track')
      ...
      //TODO window.log.warn('selectAudioInputByDeviceId failed with', e.message)
    }
...
async function createOfferAndSendIt(recipient: string) {
  try {
    makingOffer = true
    //TODO window.log.info('got createOfferAndSendIt event. creating offer')
    ...
    if (!currentCallUUID) {
      //TODO window.log.warn('cannot send offer without a currentCallUUID')
      throw new Error('cannot send offer without a currentCallUUID')
    }

    if (offer && offer.sdp) {
      const lines = offer.sdp.split(/\r?\n/)
      const lineWithFtmpIndex = lines.findIndex((f) =>
        f.startsWith('a=fmtp:111'),
      )
      ...
      window.log.info(
        `sending '${offer.type}'' with callUUID: ${currentCallUUID}`,
      )
    ...
    //TODO window.log?.error(`Error createOfferAndSendIt ${err}`)
  } finally {
    makingOffer = false
  }
}
...
async function openMediaDevicesAndAddTracks() {
  try {
    ...
    //TODO window.log.warn('openMediaDevices: ', err)
...
export async function USER_callRecipient(recipient: string) {
  ...
  await updateConnectedDevices()
  const now = Date.now()
  //TODO window?.log?.info(`starting call with ${ed25519Str(recipient)}..`)
  //TODO window.inboxStore?.dispatch(
  //   startingCallWith({
  //     pubkey: recipient,
  //   }),
  //)
  ...
  //TODO window.log.info('Sending preOffer message to ', ed25519Str(recipient))
...
const iceSenderDebouncer = _.debounce(async (recipient: string) => {
  ...
  if (!currentCallUUID) {
    //TODO window.log.warn('Cannot send ice candidates without a currentCallUUID')
    return
  }
  ...
  //TODO window.log.info(
  //   `sending ICE CANDIDATES MESSAGE to ${ed25519Str(
  //     recipient,
  //   )} about call ${currentCallUUID}`,
  // )
...
const findLastMessageTypeFromSender = (
  sender: string,
  msgType: SignalService.CallMessage.Type,
) => {
...
function handleConnectionStateChanged(pubkey: string) {
  ...
  if (
    ...
    //TODO window.inboxStore?.dispatch(callReconnecting({ pubkey }))
    ...
    //TODO window.inboxStore?.dispatch(callConnected({ pubkey }))
  }
}

function closeVideoCall() {
  //TODO window.log.info('closingVideoCall ')
...
function getCallingStateOutsideOfRedux() {
  //TODO const ongoingCallWith = window.inboxStore?.getState().call.ongoingWith as
  //   | string
  //   | undefined
  
  // const ongoingCallStatus = window.inboxStore?.getState().call
  //   .ongoingCallStatus as CallStatusEnum
  return {}
}
...
function onDataChannelReceivedMessage(ev: MessageEvent<string>) {
  try {
    ...
    //TODO window.log.warn(
    //   'onDataChannelReceivedMessage Could not parse data in event',
    //   ev,
    // )
  }
  callVideoListeners()
...
function onDataChannelOnOpen() {
  //TODO window.log.info('onDataChannelOnOpen: sending video status')
  sendVideoStatusViaDataChannel()
}
...
function createOrGetPeerConnection(withPubkey: string) {
  ...
  peerConnection.oniceconnectionstatechange = () => {
    ...
    if (
      ...
      global.setTimeout(async () => {
        //TODO window.log.info('onconnectionstatechange disconnected: restartIce()')
...
export async function USER_acceptIncomingCallRequest(fromSender: string) {
  //TODO window.log.info('USER_acceptIncomingCallRequest')
  if (currentCallUUID) {
    // window.log.warn(
  ...
  if (!lastOfferMessage.uuid) {
    //TODO window?.log?.info(
    //   'incoming call request cannot be accepted as uuid is invalid',
    // )
    return
  }
  // TODO window.inboxStore?.dispatch(answerCall({ pubkey: fromSender }))
  ...
  if (peerConnection) {
    throw new Error(
      'USER_acceptIncomingCallRequest: peerConnection is already set.',
    )
  }
  ...
  try {
    ...
    //TODO window.log?.error(`Error setting RTC Session Description ${e}`)
  }
  ...
  if (lastCandidatesFromSender) {
    //TODO window.log.info('found sender ice candicate message already sent. Using it')
...
export async function rejectCallAlreadyAnotherCall(
  ...
  //TODO window.log.info(
  //   `rejectCallAlreadyAnotherCall ${ed25519Str(fromSender)}: ${forcedUUID}`,
  // )
...
export async function USER_rejectIncomingCallRequest(fromSender: string) {
  ...
  //TODO window.log.info(
  //   `USER_rejectIncomingCallRequest ${ed25519Str(
  //     fromSender,
  //   )}: ${aboutCallUUID}`,
  // )
  if (aboutCallUUID) {
  ...
  //TODO const { ongoingCallStatus, ongoingCallWith } = getCallingStateOutsideOfRedux()
...
export async function USER_hangup(fromSender: string) {
  //TODO window.log.info('USER_hangup')

  if (!currentCallUUID) {
    //TODO window.log.warn('should not be able to hangup without a currentCallUUID')
    return
  } else {
    ...
    void getMessageQueue().sendToPubKeyNonDurably(
      PubKey.cast(fromSender),
      endCallMessage,
    )
  }

  //TODO window.inboxStore?.dispatch(endCall())
  //TODO window.log.info('sending hangup with an END_CALL MESSAGE')
 ...
 */
export async function handleCallTypeEndCall(
  sender: string,
  aboutCallUUID?: string,
) {
  //TODO window.log.info('handling callMessage END_CALL:', aboutCallUUID)
...
async function buildAnswerAndSendIt(sender: string) {
  if (peerConnection) {
    if (!currentCallUUID) {
      //TODO window.log.warn('cannot send answer without a currentCallUUID')
      return
    }
    await (peerConnection as any).setLocalDescription()
    const answer = peerConnection.localDescription
    if (!answer?.sdp || answer.sdp.length === 0) {
      //TODO window.log.warn('failed to create answer')
      return
    }
    ...
    //TODO window.log.info('sending ANSWER MESSAGE and sync')
...
export async function handleCallTypeOffer(
  ...
  try {
    ...
    //TODO window.log.info('handling callMessage OFFER with uuid: ', remoteCallUUID)

    //TODO if (!getCallMediaPermissionsSettings()) {
    ...
    if (ignoreOffer) {
      //TODO window.log?.warn('Received offer when unready for offer; Ignoring offer.')
      return
    }

    if (
      peerConnection &&
      remoteCallUUID === currentCallUUID &&
      currentCallUUID
    ) {
      //TODO window.log.info('Got a new offer message from our ongoing call')
      ...
      //TODO window.inboxStore?.dispatch(incomingCall({ pubkey: sender }))

      // show a notification
      const callerConvo = getConversationController().get(sender)
      const convNotif =
        callerConvo?.get('triggerNotificationsFor') || 'disabled'
      if (convNotif === 'disabled') {
        //TODO window?.log?.info(
        //   'notifications disabled for convo',
        //   ed25519Str(sender),
        // )
      } else if (callerConvo) {
    ...
    //TODO window.log?.error(`Error handling offer message ${err}`)
  }
}
...
export async function handleCallTypeAnswer(
  ...
  if (!callMessage.sdps || callMessage.sdps.length === 0) {
    //TODO window.log.warn(
    //   'cannot handle answered message without signal description proto sdps',
    // )
    return
  }
  const callMessageUUID = callMessage.uuid
  if (!callMessageUUID || callMessageUUID.length === 0) {
    //TODO window.log.warn('handleCallTypeAnswer has no valid uuid')
    return
  }
  ...
  if (sender === UserUtils.getOurPubKeyStrFromCache()) {
    ...
    //TODO window.log.info(
    //   `handling callMessage ANSWER from ourself about call ${callMessageUUID}`,
    // )
    ...
    if (callMessageUUID !== currentCallUUID) {
      ...
      if (foundOwnerOfCallUUID) {
        ...
        //TODO if (ongoingCallStatus && ongoingCallWith === foundOwnerOfCallUUID) {
        //   closeVideoCall()
        // }
    ...
    //TODO window.log.info(`handling callMessage ANSWER from ${callMessageUUID}`)
  }
  const cachedMessage = getCachedMessageFromCallMessage(
    callMessage,
    envelopeTimestamp,
  ...
  if (!peerConnection) {
    //TODO window.log.info('handleCallTypeAnswer without peer connection. Dropping')
    return
  }
  //TODO window.inboxStore?.dispatch(
  //   answerCall({
  ...
  try {
    ...
    //TODO window.log.warn('setRemoteDescriptio failed:', e)
  } finally {
    isSettingRemoteAnswerPending = false
  }
}
...
export async function handleCallTypeIceCandidates(
  ...
  if (!callMessage.sdps || callMessage.sdps.length === 0) {
    //TODO window.log.warn('cannot handle iceCandicates message without candidates')
    return
  }
  const remoteCallUUID = callMessage.uuid
  if (!remoteCallUUID || remoteCallUUID.length === 0) {
    //TODO window.log.warn('handleCallTypeIceCandidates has no valid uuid')
    return
  }
  //TODO window.log.info('handling callMessage ICE_CANDIDATES')
  const cachedMessage = getCachedMessageFromCallMessage(
    callMessage,
    envelopeTimestamp,
  )
...
async function addIceCandidateToExistingPeerConnection(
  callMessage: SignalService.CallMessage,
) {
  if (peerConnection) {
    ...
    for (let index = 0; index < callMessage.sdps.length; index++) {
      ...
      try {
        ...
        if (!ignoreOffer) {
         //TODO window.log?.warn('Error handling ICE candidates message', err)
        }
      }
    }
  } else {
    //TODO window.log.info(
    //   'handleIceCandidatesMessage but we do not have a peerconnection set',
    // )
  }
}
...
export async function handleOtherCallTypes(
  ...
  const remoteCallUUID = callMessage.uuid
  if (!remoteCallUUID || remoteCallUUID.length === 0) {
    //TODO window.log.warn('handleOtherCallTypes has no valid uuid')
    return
  }
  const cachedMessage = getCachedMessageFromCallMessage(
    callMessage,
    ...
    sender.forEach((msgs) => {
      for (const msg of msgs.reverse()) {
        if (
          ...
          //TODO window.inboxStore?.dispatch(incomingCall({ pubkey: key }))
          break
        }
      }
    })
