  ...
  const contacts = contactsModels.map((c) => {
    try {
      ...
      return new ConfigurationMessageContact({
        ...
        profileKey: !profileKeyForContact?.length
          ? undefined
          : profileKeyForContact,
        isApproved: c.isApproved(),
        isBlocked: c.isBlocked(),
        didApproveMe: c.didApproveMe(),
      })
    } catch (e) {
      //TODO window?.log.warn('getValidContacts', e)
      return null
    }
  })
...
export const getCurrentConfigurationMessage = async (
  ...
  const ourProfileKeyHex =
    getConversationController()
      ...
      ?.get('profileKey') || null
  const profileKey = ourProfileKeyHex
    ? fromHexToArray(ourProfileKeyHex)
    : undefined
...
export const buildSyncMessage = (
  ...
  if (
    ...
    //TODO window?.log?.warn('buildSyncMessage with something else than a DataMessage')
  }
