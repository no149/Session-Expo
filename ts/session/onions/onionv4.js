...
const decodeV4Response = (snodeResponse) => {
    ...
    if (!(binary &&
        ...
        window?.log?.error('decodeV4Response: response is missing prefix and suffix characters - Dropping response');
        return;
    }
    try {
        ...
        switch (bodyContentType) {
            ...
            case 'text/html; charset=utf-8':
                try {
                    window?.log?.warn('decodeV4Response - received raw body of type "text/html; charset=utf-8": ', (0, libsodium_wrappers_sumo_1.to_string)(bodyBinary));
                }
                catch (e) {
                    window?.log?.warn('decodeV4Response - received raw body of type "text/html; charset=utf-8" but not a string');
                }
                break;
            default:
                window?.log?.warn('decodeV4Response - No or unknown content-type information for response: ', bodyContentType);
        }
        return {
            metadata: infoJson,
            body: bodyParsed,
