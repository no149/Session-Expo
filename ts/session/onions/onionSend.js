...
const sendViaOnionV4ToNonSnodeWithRetries = async (destinationX25519Key, url, fetchOptions, throwErrors, abortSignal) => {
    ...
    if (window.sessionFeatureFlags?.debug.debugNonSnodeRequests) {
        window.log.info('sendViaOnionV4ToNonSnodeWithRetries: buildSendViaOnionPayload returned ', JSON.stringify(payloadObj));
    }
    ...
    try {
        result = await (0, p_retry_1.default)(async () => {
            ...
            if (window.sessionFeatureFlags?.debug.debugNonSnodeRequests) {
                window.log.info('sendViaOnionV4ToNonSnodeWithRetries: getOnionPathForSending returned', JSON.stringify(pathNodes));
            }
            if (!pathNodes) {
            ...
            if (window.sessionFeatureFlags?.debug.debugNonSnodeRequests) {
                window.log.info('sendViaOnionV4ToNonSnodeWithRetries: sendOnionRequestHandlingSnodeEject returned: ', JSON.stringify(onionV4Response));
            }
            ...
            if (foundStatusCode < 200 || foundStatusCode > 299) {
                ...
                if (foundStatusCode === 404) {
                    window.log.warn(`Got 404 while sendViaOnionV4ToNonSnodeWithRetries with url:${url}. Stopping retries`);
            ...
            minTimeout: 100,
            onFailedAttempt: e => {
                window?.log?.warn(`sendViaOnionV4ToNonSnodeRetryable attempt #${e.attemptNumber} failed. ${e.retriesLeft} retries left...: ${e.message}`);
            },
        });
    }
    catch (e) {
...
async function getBinaryViaOnionV4FromFileServer(sendOptions) {
    ...
    if (window.sessionFeatureFlags?.debug.debugFileServerRequests) {
        window.log.info(`getBinaryViaOnionV4FromFileServer fsv2: "${builtUrl}; got:`, JSON.stringify(res));
    }
    return res;
}
