...
export class ConversationController {
  ...
  public get(id: string): ConversationModel {
    if (!this._initialFetchComplete) {
      throw new Error(
        'getConversationController().get() needs complete initial fetch',
      )
    }
  ...
  public getOrThrow(id: string): ConversationModel {
    ...
    throw new Error(
      `Conversation ${id} does not exist on getConversationController().get()`,
    )
  }
  ...
  public async deleteBlindedContact(blindedId: string) {
    ...
    //TODO window.log.info(`deleteBlindedContact with ${blindedId}`)
    const conversation = this.conversations.get(blindedId)
    if (!conversation) {
      //TODO window.log.warn(`deleteBlindedContact no such convo ${blindedId}`)
      return
    }
  ...
  public async deleteContact(id: string) {
    if (!this._initialFetchComplete) {
      throw new Error(
        'getConversationController().deleteContact() needs complete initial fetch',
      )
    }

    //TODO window.log.info(`deleteContact with ${id}`)

    const conversation = this.conversations.get(id)
    if (!conversation) {
      //TODO window.log.warn(`deleteContact no such convo ${id}`)
      return
    }

    // Closed/Medium group leaving
    if (conversation.isClosedGroup()) {
      //TODO window.log.info(`deleteContact ClosedGroup case: ${id}`)
      await conversation.leaveClosedGroup()
      // open group v2
    } else if (conversation.isOpenGroupV2()) {
      //TODO window?.log?.info('leaving open group v2', conversation.id)
      ...
      if (roomInfos) {
        ...
        try {
          ...
          //TODO window?.log?.info('removeV2OpenGroupRoom failed:', e)
        }
      }
    }

    // those are the stuff to do for all contact types
    //TODO window.log.info(`deleteContact destroyingMessages: ${id}`)

    await deleteAllMessagesByConvoIdNoConfirmation(conversation.id)
    //TODO window.log.info(`deleteContact message destroyed: ${id}`)
    ...
    if (conversation.isPrivate()) {
      ...
      //TODO window.log.info(`deleteContact !isPrivate, removing convo from DB: ${id}`)

      await Data.removeConversation(id)
      //TODO      window.log.info(`deleteContact !isPrivate, convo removed from DB: ${id}`)
      ...
      // }
      // window.log.info(
      //   `deleteContact !isPrivate, convo removed from store: ${id}`,
      // )
    }
  }
  ...
  public async load() {
    ...
    const load = async () => {
      try {
        ...
        window?.log?.info(
          `ConversationController: done with initial fetch in ${
            Date.now() - start
          }ms.`,
        )
      } catch (error) {
