...
class MessageQueue {
    ...
    async sendToOpenGroupV2(message, roomInfos, blinded, filesToLink) {
        try {
            if (message.reaction) {
            ...
            window?.log?.warn(`Failed to send message to open group: ${roomInfos.serverUrl}:${roomInfos.roomId}:`, e);
    ...
    async sendToOpenGroupV2BlindedRequest(encryptedContent, roomInfos, message, recipientBlindedId) {
        try {
            if (!types_1.PubKey.hasBlindedPrefix(recipientBlindedId)) {
            ...
            window?.log?.warn(`Failed to send message to open group: ${roomInfos.serverUrl}:${roomInfos.roomId}:`, e.message);
            await MessageSentHandler_1.MessageSentHandler.handleMessageSentFailure(message, e || new Error('Failed to send message to open group.'));
        }
    }
