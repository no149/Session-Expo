...
async function blindedED25519Signature(
  ...
  const r = sodium.crypto_core_ed25519_scalar_reduce(
    sha512Multipart([Hrh, kA, messageParts]),
  )
...
const getBlindingValues = (
  ...
  const k = sodium.crypto_core_ed25519_scalar_reduce(
    sodium.crypto_generichash(64, serverPK),
  )
  ...
  if (a.length > 32) {
    window.log.warn(
      'length of signing key is too long, cutting to 32: oldlength',
      length,
    )
    a = a.slice(0, 32)
  }
...
const encryptBlindedMessage = async (options: {
  ...
  const aBlindingValues = SogsBlinding.getBlindingValues(
    serverPubKey,
    senderSigningKey,
    sodium,
  )
...
async function decryptWithSessionBlindingProtocol(
  ...
  const otherKeyBytes = from_hex(
    PubKey.removePrefixIfNeeded(otherBlindedPublicKey),
  )
