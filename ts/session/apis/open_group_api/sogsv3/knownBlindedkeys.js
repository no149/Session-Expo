...
exports.KNOWN_BLINDED_KEYS_ITEM = 'KNOWN_BLINDED_KEYS_ITEM';
...
async function loadKnownBlindedKeys() {
    ...
    const fromDb = await data_1.Data.getItemById(exports.KNOWN_BLINDED_KEYS_ITEM);
...
async function writeKnownBlindedKeys() {
    if (cachedKnownMapping && cachedKnownMapping.length) {
        await data_1.Data.createOrUpdateItem({
            id: exports.KNOWN_BLINDED_KEYS_ITEM,
...
async function addCachedBlindedKey({ blindedId, serverPublicKey, realSessionId, }) {
    ...
    if (foundIndex >= 0) {
        if (assertLoadedCache[foundIndex].realSessionId !== realSessionId) {
            window.log.warn(`overriding cached blinded mapping for ${assertLoadedCache[foundIndex].realSessionId} with ${realSessionId} on ${serverPublicKey}`);
...
function tryMatchBlindWithStandardKey(standardSessionId, blindedSessionId, serverPubKey, sodium) {
    ...
    if (!blindedSessionId.startsWith(types_1.KeyPrefixType.blinded)) {
...
function findNotCachedBlindingMatch(blindedId, serverPublicKey, sodium) {
    ...
    const foundConvoMatchingBlindedPubkey = (0, conversations_1.getConversationController)()
        .getConversations()
        .filter(m => m.isPrivate() && m.isApproved() && !types_1.PubKey.hasBlindedPrefix(m.id))
        .find(m => {
...
function isUsAnySogsFromCache(blindedOrNakedId) {
    ...
    if (!types_1.PubKey.hasBlindedPrefix(blindedOrNakedId)) {
...
function getUsBlindedInThatServer(convo) {
    ...
    if (!room || !(0, sogsV3Capabilities_1.roomHasBlindEnabled)(room) || !room.serverPublicKey) {
        return undefined;
    }
...
function findNotCachedBlindedConvoFromUnblindedKey(unblindedID, serverPublicKey, sodium) {
    if (types_1.PubKey.hasBlindedPrefix(unblindedID)) {
    ...
    const foundConvosForThisServerPk = (0, conversations_1.getConversationController)()
        .getConversations()
        .filter(m => m.isPrivate() && types_1.PubKey.hasBlindedPrefix(m.id) && m.isActive())
        .filter(m => {
...
async function findCachedBlindedMatchOrLookItUp(blindedId, serverPubKey, sodium) {
    if (!types_1.PubKey.hasBlindedPrefix(blindedId)) {
        return blindedId;
    }
...
async function findCachedOurBlindedPubkeyOrLookItUp(serverPubKey, sodium) {
    ...
    if (types_1.PubKey.hasBlindedPrefix(ourNakedSessionID)) {
