...
async function getOpenGroupHeaders(data) {
    ...
    if (blinded) {
        ...
        pubkey = `${types_1.KeyPrefixType.blinded}${(0, String_1.toHex)(kA)}`;
    }
    else {
...
const getBlindedPubKey = (serverPK, signingKeys, sodium) => {
    ...
    return `${types_1.KeyPrefixType.blinded}${(0, libsodium_wrappers_sumo_1.to_hex)(blindedPubKeyBytes.publicKey)}`;
};
