...
export function tryMatchBlindWithStandardKey(
  ...
  try {
    ...
    const sessionIdNoPrefix = PubKey.removePrefixIfNeeded(
      PubKey.cast(standardSessionId).key,
    )
    const blindedIdNoPrefix = PubKey.removePrefixIfNeeded(
      PubKey.cast(blindedSessionId).key,
    )
    ...
    const match =
      isEqual(blindedIdNoPrefix, to_hex(pk1)) ||
      isEqual(blindedIdNoPrefix, to_hex(pk2))
    ...
    window.log.warn(
      'Failed to do crypto tryMatchBlindWithStandardKey with ',
      e.message,
    )
    return false
  }
...
function findNotCachedBlindingMatch(
  ...
  if (isNonBlindedKey(blindedId)) {
    throw new Error(
      'findNotCachedBlindingMatch blindedId is supposed to be blinded',
    )
  }
  ...
  const foundConvoMatchingBlindedPubkey = getConversationController()
    .getConversations()
    .filter(
      (m) => m.isPrivate() && m.isApproved() && !PubKey.hasBlindedPrefix(m.id),
    )
    .find((m) => {
      return tryMatchBlindWithStandardKey(
        m.id,
        blindedId,
        serverPublicKey,
        sodium,
 ...
 */
export function getUsBlindedInThatServer(
  convo: ConversationModel | string,
): string | undefined {
  if (!convo) {
    return undefined
  }
  ...
  const room = OpenGroupData.getV2OpenGroupRoom(
    isString(convo) ? convo : convo.id,
  )
  ...
  const found = assertLoaded().find(
    (m) =>
      m.serverPublicKey === room.serverPublicKey && m.realSessionId === usNaked,
  )
...
function findNotCachedBlindedConvoFromUnblindedKey(
  ...
  const foundConvosForThisServerPk =
    getConversationController()
      .getConversations()
      .filter(
        (m) => m.isPrivate() && PubKey.hasBlindedPrefix(m.id) && m.isActive(),
      )
      .filter((m) => {
        return tryMatchBlindWithStandardKey(
          unblindedID,
...
export async function findCachedBlindedMatchOrLookItUp(
  ...
  const realSessionIdFound = findNotCachedBlindingMatch(
    blindedId,
    serverPubKey,
    sodium,
  )
...
export function findCachedBlindedIdFromUnblinded(
  ...
  const found = assertLoaded().find(
    (m) =>
      m.serverPublicKey === serverPubKey && m.realSessionId === unblindedId,
  )
...
export async function findCachedOurBlindedPubkeyOrLookItUp(
  ...
  if (PubKey.hasBlindedPrefix(ourNakedSessionID)) {
    throw new Error(
      'findCachedBlindedIdFromUnblindedOrLookItUp needs a unblindedID',
    )
  }
...
export function getCachedNakedKeyFromBlindedNoServerPubkey(
  blindedId: string,
): string | undefined {
  if (isNonBlindedKey(blindedId)) {
    return blindedId
...
export function findCachedBlindedMatchOrLookupOnAllServers(
  ...
  if (PubKey.hasBlindedPrefix(unblindedId)) {
    throw new Error(
      'findCachedBlindedMatchOrLookupOnAllServers needs an unblindedId',
    )
  }
  ...
  let matchingServerPubkeyWithThatBlindedId = flatten(
    allServerPubkeys.map((serverPk) => {
      return findNotCachedBlindedConvoFromUnblindedKey(
        unblindedId,
        serverPk,
        sodium,
      )
