 ...
 */
function getSogsConvoOrReturnEarly(
  serverUrl: string,
  roomId: string,
  ...
  if (!convoId) {
    window.log.info(
      `getSogsConvoOrReturnEarly: convoId not built with ${serverUrl}: ${roomId}`,
    )
    return null
  }
  ...
  if (!foundConvo.isOpenGroupV2()) {
    window.log.info(
      'getSogsConvoOrReturnEarly: convo not an opengroup: ',
      convoId,
    )
    return null
  }
...
async function handlePollInfoResponse(
  ...
  if (statusCode !== 200) {
    window.log.info(
      'handlePollInfoResponse subRequest status code is not 200:',
      statusCode,
    )
    return
  }
  ...
  const stillPolledRooms = OpenGroupData.getV2OpenGroupRoomsByServerUrl(
    serverUrl,
  )

  if (
    !stillPolledRooms?.some(
      (r) => r.roomId === token && r.serverUrl === serverUrl,
    )
  ) {
...
async function filterOutMessagesInvalidSignature(
  ...
  window.log.info(
    `[perf] verifyAllSignatures took ${Date.now() - startVerify}ms.`,
  )
...
const handleSogsV3DeletedMessages = async (
  ...
  try {
    ...
    const messageIds = await Data.getMessageIdsFromServerIds(
      allIdsRemoved,
      convo.id,
    )
...
const handleMessagesResponseV4 = async (
  ...
  try {
    ...
    if (
      !stillPolledRooms?.some(
        (r) => r.roomId === roomId && r.serverUrl === serverUrl,
      )
    ) {
      window.log.info(
        `handleMessagesResponseV4: we are no longer polling for ${roomId}: skipping`,
      )
      return
    }
    const convoId = getOpenGroupV2ConversationId(serverUrl, roomId)
    ...
    if (!isArray(messages)) {
      window.log.warn(
        "handleMessagesResponseV4: didn't get an object from batch poll",
      )
      return
    }
    ...
    const roomDetails: OpenGroupRequestCommonType = pick(
      roomInfos,
      'serverUrl',
      'roomId',
    )
    ...
    for (let index = 0; index < messagesFilteredBlindedIds.length; index++) {
      ...
      if (newMessage.session_id) {
        const unblindedIdFound = getCachedNakedKeyFromBlindedNoServerPubkey(
    ...
    for (
      let index = 0;
      index < messagesWithResolvedBlindedIdsIfFound.length;
      index++
    ) {
    ...
    const roomInfosRefreshed = OpenGroupData.getV2OpenGroupRoom(
      roomInfos.conversationId,
    )
    if (
      !roomInfosRefreshed ||
      !roomInfosRefreshed.serverUrl ||
      !roomInfosRefreshed.roomId
    ) {
      ...
      if (groupConvo && groupConvo.isOpenGroupV2()) {
        for (const messageWithReaction of messagesWithReactions) {
          if (isEmpty(messageWithReaction.reactions)) {
             ...
             */
            if (
              sogsRollingDeletions.hasMessageDeletedId(
                conversationId,
                messageWithReaction.id,
              )
            ) {
              continue
            }
          ...
          void groupConvo.queueJob(async () => {
            await processMessagesUsingCache(
              serverUrl,
              roomId,
              messageWithReaction,
            )
...
async function handleInboxOutboxMessages(
  ...
  if (
    !inboxOutboxResponse ||
    !isArray(inboxOutboxResponse) ||
    inboxOutboxResponse.length === 0
  ) {
    //nothing to do
    return
  }
    ...
    try {
      ...
      if (isOutbox) {
        ...
        const unblindedIDOrBlinded =
          (await findCachedBlindedMatchOrLookItUp(
            recipient,
            serverPubkey,
            sodium,
          )) || recipient
        ...
        if (contentDecoded.dataMessage) {
          ...
          const serverConversationId = OpenGroupData.getV2OpenGroupRoomsByServerUrl(
            serverUrl,
          )?.[0].conversationId
          ...
          await outboxConversationModel.setOriginConversationID(
  ...
  if (!rooms || !rooms.length) {
    window?.log?.error(
      'handleInboxOutboxMessages - Found no rooms with matching server url',
    )
    return
  }
  ...
  const maxInboxOutboxId = inboxOutboxResponse.length
    ? Math.max(
        ...inboxOutboxResponse.map((inboxOutboxItem) => inboxOutboxItem.id),
      )
    : undefined || undefined
  ...
  if (batchPollResults && isArray(batchPollResults.body)) {
    ...
    for (let index = 0; index < batchPollResults.body.length; index++) {
      ...
      switch (responseType) {
        ...
        default:
          window.log.error(
            'No matching subrequest response body for type: ',
            responseType,
          )
      }
    }
