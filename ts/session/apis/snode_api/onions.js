...
exports.Onions = exports.snodeHttpsAgent = exports.STATUS_NO_STATUS = exports.CLOCK_OUT_OF_SYNC_MESSAGE_ERROR = exports.ERROR_421_HANDLED_RETRY_REQUEST = exports.NEXT_NODE_NOT_FOUND_PREFIX = exports.buildErrorMessageWithFailedCode = exports.was404Error = exports.OXEN_SERVER_ERROR = exports.resetSnodeFailureCount = void 0;
    ...
    for (let i = firstPos; i > -1; i -= 1) {
        ...
        if (relayingToFinalDestination && finalRelayOptions) {
            ...
            if (!relayingToFinalDestination) {
                ...
                if (!pubkeyHex) {
                    window?.log?.error('loki_rpc:::buildOnionGuardNodePayload - no ed25519 for', nodePath[i + 1], 'path node', i + 1);
                }
            }
            dest = {
        ...
        try {
            ...
            window?.log?.error('loki_rpc:::buildOnionGuardNodePayload - encryptForRelayV2 failure', e.code, e.message);
            throw e;
        }
    }
    return ctxes;
...
async function processAnyOtherErrorOnPath(status, guardNodeEd25519, ciphertext, associatedWith) {
    if (status !== 200) {
        ...
        if (status === 404 || status === 400) {
            window?.log?.warn('processAnyOtherErrorOnPathgot 404 or 400, probably a dead sogs. Skipping bad path update');
            return;
        }
...
async function processOnionResponse({ response, symmetricKey, guardNode, abortSignal, associatedWith, destinationSnodeEd25519, }) {
    ...
    if (!ciphertext) {
        window?.log?.warn('[path] sessionRpc::processingOnionResponse - Target node return empty ciphertext');
    ...
    try {
        ...
        if (symmetricKey) {
            window?.log?.error('[path] sessionRpc::processingOnionResponse - symmetricKey', (0, String_1.toHex)(symmetricKey));
        }
        if (ciphertextBuffer) {
            window?.log?.error('[path] sessionRpc::processingOnionResponse - ciphertextBuffer', (0, String_1.toHex)(ciphertextBuffer));
        }
    ...
    try {
        ...
        window?.log?.error(`[path] sessionRpc::processingOnionResponse - Rethrowing error ${e.message}'`);
        throw e;
    }
}
...
async function processOnionResponseV4({ response, symmetricKey, abortSignal, guardNode, destinationSnodeEd25519, associatedWith, }) {
    ...
    if (!cipherText) {
        window?.log?.warn('[path] sessionRpc::processOnionResponseV4 - Target node/path return empty ciphertext');
        throw new Error('Target node return empty ciphertext');
    }
    await processOnionRequestErrorOnPath(response?.status || exports.STATUS_NO_STATUS, cipherText, guardNode.pubkey_ed25519, destinationSnodeEd25519, associatedWith);
...
async function handle421InvalidSwarm({ body, destinationSnodeEd25519, associatedWith, }) {
    ...
    try {
        ...
        if (parsedBody?.snodes?.length) {
            window?.log?.warn('Wrong swarm, now looking at snodes', parsedBody.snodes.map((s) => (0, onionPath_1.ed25519Str)(s.pubkey_ed25519)));
...
async function sendOnionRequestHandlingSnodeEject({ destSnodeX25519, finalDestOptions, nodePath, abortSignal, associatedWith, finalRelayOptions, useV4, throwErrors, }) {
    ...
    try {
        ...
        if (window.sessionFeatureFlags?.debug.debugOnionRequests) {
            window.log.info(`sendOnionRequestHandlingSnodeEject: sendOnionRequestNoRetries: useV4:${useV4} destSnodeX25519:${destSnodeX25519}; \nfinalDestOptions:${JSON.stringify(finalDestOptions)}; \nfinalRelayOptions:${JSON.stringify(finalRelayOptions)}\n\n result: ${JSON.stringify(result)}`);
        }
    ...
    try {
        ...
        if (isRequestToSnode) {
            ...
            if (!(0, lodash_1.isString)(finalDestOptions.body)) {
                window.log.warn('snoderpc calls should only take body as string: ', typeof finalDestOptions.body);
        ...
        window?.log?.error('sendOnionRequestNoRetries - encryptForPubKey failure [', e.code, e.message, '] destination X25519', destX25519hex.substring(0, 32), '...', destX25519hex.substring(32));
        throw e;
    }
