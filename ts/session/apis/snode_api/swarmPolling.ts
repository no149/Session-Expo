...
export class SwarmPolling {
  ...
  private readonly lastHashes: Record<
    string,
  ...
  public addGroupId(pubkey: PubKey) {
    if (
      this.groupPolling.findIndex((m) => m.pubkey.key === pubkey.key) === -1
    ) {
      window?.log?.info(
        'Swarm addGroupId: adding pubkey to polling',
        pubkey.key,
      )
  ...
  public removePubkey(pk: PubKey | string) {
    ...
    this.groupPolling = this.groupPolling.filter(
      (group) => !pubkey.isEqual(group.pubkey),
    )
  }
  ...
  public async pollForAllKeys() {
    ...
    const groupPromises = this.groupPolling.map(async (group) => {
      ...
      const loggingId =
        getConversationController().get(group.pubkey.key)?.idForLogging() ||
        group.pubkey.key
  ...
  public async pollOnceForKey(
    ...
    const alreadyPolled = swarmSnodes.filter(
    ...
    if (isGroup && arrayOfResults?.length) {
      window?.log?.info(
        `Polled for group(${ed25519Str(pubkey.key)}):, got ${
          messages.length
        } messages back.`,
      )
      ...
      if (messages.length >= 95) {
        ...
        lastPolledTimestamp =
          Date.now() - SWARM_POLLING_TIMEOUT.INACTIVE - 5 * 1000
      }
    ...
    try {
      return await pRetry(
        async () => {
          ...
          const messages = await retrieveNextMessages(
            node,
            prevHash,
            pkStr,
            namespace,
  ...
  private async getLastHash(
    ...
    if (!this.lastHashes[nodeEdKey]?.[pubkey]?.[namespace]) {
      const lastHash = await Data.getLastHashBySnode(
        pubkey,
        nodeEdKey,
        namespace,
      )
