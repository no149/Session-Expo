import { default as insecureNodeFetch, AxiosResponse } from 'axios'
...
export const was404Error = (error: Error) =>
  error.message.includes(errorContent404)

export const buildErrorMessageWithFailedCode = (
  prefix: string,
  code: number,
  suffix: string,
) => `${prefix}: ${code} ${suffix}`
...
async function encryptForRelayV2(
  ...
  if (!destination.host && !destination.destination) {
    //TODO window?.log?.warn('loki_rpc::encryptForRelayV2 - no destination', destination);
  }
...
async function buildOnionCtxs(
  ...
  for (let i = firstPos; i > -1; i -= 1) {
    ...
    try {
      ...
      const ctx = await encryptForRelayV2(
...
function processOxenServerError(_statusCode: number, body?: string) {
  if (body === OXEN_SERVER_ERROR) {
    //TODO window?.log?.warn('[path] Got Oxen server Error. Not much to do if the server has troubles.');
...
async function handleNodeNotFound({
  ...
  //TODO window?.log?.warn('Handling NODE NOT FOUND with: ', shortNodeNotFound);
...
async function processAnyOtherErrorOnPath(
  ...
  if (status !== 200) {
    //TODO window?.log?.warn(`[path] Got status: ${status}`);
...
async function processAnyOtherErrorAtDestination(
  ...
  if (
    ...
    //TODO window?.log?.warn(`[path] Got status at destination: ${status}`);
...
async function processOnionRequestErrorOnPath(
  ...
  if (httpStatusCode !== 200) {
    //TODO window?.log?.warn('processOnionRequestErrorOnPath:', ciphertext);
  }
...
function processAbortedRequest(abortSignal?: AbortSignal) {
  if (abortSignal?.aborted) {
    //TODO  window?.log?.warn('[path] Call aborted');
    // this will make the pRetry stop
...
async function decodeOnionResult(
  ...
  const ciphertextBuffer = await callUtilsWorker(
...
async function processOnionResponse({
  ...
  try {
    ...
    //TODO window?.log?.error('[path] sessionRpc::processingOnionResponse - decode error', e);
    if (symmetricKey) {
      //TODO window?.log?.error(
  ...
  if (debug) {
    //TODO window?.log?.debug('sessionRpc::processingOnionResponse - plaintext', plaintext);
  }

  try {
    const jsonRes = JSON.parse(plaintext, (_key, value) => {
      if (typeof value === 'number' && value > Number.MAX_SAFE_INTEGER) {
        //TODO window?.log?.warn('Received an out of bounds js number');
      }
      return value
...
async function processNoSymmetricKeyError(
  ...
  if (!symmetricKey) {
    ...
    //TODO window?.log?.error(errorMsg);
...
async function processOnionResponseV4({
  ...
  processAbortedRequest(abortSignal)
  const validSymmetricKey = await processNoSymmetricKeyError(
    guardNode,
    symmetricKey,
  )
...
export const snodeHttpsAgent = new https.HttpsProxyAgent({
...
async function handle421InvalidSwarm({
  ...
  if (!destinationSnodeEd25519 || !associatedWith) {
    ...
    throw new Error(
      'status 421 without a final destination or no associatedWith makes no sense',
    )
  }
  //TODO  window?.log?.info(`Invalidating swarm for ${ed25519Str(associatedWith)}`);
...
async function sendOnionRequestHandlingSnodeEject({
  ...
  try {
    ...
    //TODO window?.log?.warn('sendOnionRequestNoRetries error message: ', e.message);
  ...
  const destinationSnodeEd25519 =
    (isFinalDestinationSnode(finalDestOptions) &&
...
function throwIfInvalidV4RequestInfos(
  request: FinalDestOptions,
 ...
 */
function isFinalDestinationSnode(
  options: FinalDestOptions,
...
const sendOnionRequestNoRetries = async ({
  ...
  const finalDestOptions = cloneDeep(
    omit(finalDestOptionsOri, ['destination_ed25519_hex']),
  )
  if (typeof destX25519hex !== 'string') {
    //TODO window?.log?.warn('destX25519hex was not a string');
  ...
  try {
    const bodyString = isString(finalDestOptions.body)
      ? finalDestOptions.body
      : null
    const bodyBinary =
      !isString(finalDestOptions.body) && finalDestOptions.body
        ? finalDestOptions.body
        : null
    if (isRequestToSnode) {
      if (useV4) {
      ...
      const bodyEncoded = bodyString
        ? textEncoder.encode(bodyString)
        : bodyBinary
      if (!bodyEncoded) {
  ...
  const targetEd25519hex =
    (isFinalDestinationSnode(finalDestOptionsOri) &&
      finalDestOptionsOri.destination_ed25519_hex) ||
    undefined
...
function getPathString(
  pathObjArr: Array<{ ip: string; port: number }>,
): string {
...
async function lokiOnionFetch({
  ...
  try {
    ...
    //TODO window?.log?.warn('onionFetchRetryable failed ', e.message);
    ...
    if (e?.message === CLOCK_OUT_OF_SYNC_MESSAGE_ERROR) {
      //TODO  window?.log?.warn('Its a clock out of sync error ');
      throw new pRetry.AbortError(CLOCK_OUT_OF_SYNC_MESSAGE_ERROR)
    }
    throw e
  }
